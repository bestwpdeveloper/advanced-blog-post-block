...
class bwdabpbSupport {
    ...
    public function __construct() {
        $this->bwdabpb_fetch_api_key();
        add_action( 'admin_notices', [$this,'bwdabpb_admin_updates_plugin_notice'] );
        add_action('admin_post_handle_bwdabpb_email_subscription', [$this, 'handle_bwdabpb_email_subscription']);
    }

    public function bwdabpb_admin_updates_plugin_notice() {
        if (!get_option('bwdabpb_email_subscription_notice_shown', false)) {
            ...
            echo '<input type="hidden" name="action" value="handle_bwdabpb_email_subscription">';
            echo '<button type="submit" class="button button-primary bwdabpb-notice-btn">Hide Notice</button>';
            echo '</form>';
            echo '</div>';
        }
    }
    
    private function bwdabpb_fetch_api_key() {
    ...
    public function handle_bwdabpb_email_subscription() {
        if (isset($_POST['email']) && is_email($_POST['email'])) {
            $email = sanitize_email($_POST['email']);
            $this->add_to_mailchimp($email);
            update_option('bwdabpb_email_subscription_notice_shown', true);
